...
class ModelsCode extends CCodeModel
{
	public $tablePrefix;
	public $tableName;
	public $modelClass;
	...
	public function generateRules($table)
	{
		...
		if($safe!==array())
			$rules[]="array('".implode(', ',$safe)."', 'safe')";
                ...
                $newRules = $this->getAdditionalFieldRules($table); // receiva all aditional velidation rules
		return array_merge($rules, $newRules);
	}

        protected function getAdditionalFieldRules($table)
        {
            $rules = array();
            
            $module = Yii::app()->controller->module;
            foreach($table->columns as $column) {
                if(!in_array($column->name, $module->protectedFileds)) {
                    $modelFiled = CckNodeField::model()->findByAttributes( array('field_name' => $column->name));
                    $modelFiled->appayExtraParams();
                echo "<pre>";
                var_dump($modelFiled->node_id);
                var_dump($modelFiled->modelAttributes);
                var_dump($modelFiled->extra);
                echo "</pre>";
                    die;
                    if(isset($modelFiled->extra["attributes"]) && is_array($modelFiled->extra["attributes"])) {
                        foreach($modelFiled->extra["attributes"] as $key => $attr) {
                            $rule = "";
                            echo "<pre>";
                            var_dump($attr);
                            echo "</pre>";
                            if($modelFiled->$key != 0 && isset($attr["widgetParams"]["fields"]) && is_array($attr["widgetParams"]["fields"])) {
                                $rule = str_replace('{{_field_name_}}', $column->name, $attr["forValidate"]);
                                foreach($attr["widgetParams"]["fields"] as $k => $v) {
                                    $rule = str_replace('{{_'.$k.'_}}', $v['value'], $rule);
                                }
                                echo "<pre>";
                                var_dump($rule);
                                echo "</pre>";
...
//                            echo "</pre>";
//                            die;
//                            if($modelFiled->$key != 0 && isset($attr["forValidate"], $attr["forValidate"]["name"])) {
//                                $param = (isset($attr["forValidate"]["paramName"]))? "'".$attr["forValidate"]["paramName"]."' => '".$attr["value"]."'": "";
//                                $rules[] = "array('".$column->name."', '".$attr["forValidate"]["name"]."', $param)";
//
//                            }
                        }
                    }
